...
contract LiquidationLibTest is Test {
    ...
    function test_ffi_liquidationReward(uint256 burnAmount, uint256 tcapPrice, uint256 collateralPrice, uint64 liquidationFee) public {
        burnAmount = bound(burnAmount, 0, 1000e18);
        tcapPrice = bound(tcapPrice, 0, 10e18);
        ...
        uint256 liquidationReward = LiquidationLib.liquidationReward(burnAmount, tcapPrice, collateralPrice, liquidationFee);
    ...
    function test_ffi_tokensRequiredForTargetHealthFactor(
        ...
        bytes memory res = vm.ffi(cmds);
        uint256 tokensRequired = LiquidationLib.tokensRequiredForTargetHealthFactor(
            targetHealthFactor, mintAmount, tcapPrice, collateralAmount, collateralPrice, liquidationPenalty, collateralDecimals
        );
