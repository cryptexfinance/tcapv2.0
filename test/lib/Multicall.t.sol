...
import {MockMulticall, RevertContract} from "../mock/MockMulticall.sol";
...
contract MulticallTest is Test {
    ...
    function test_multicall_firstRevert() public {
        bytes[] memory calls = new bytes[](2);
        calls[0] = abi.encodeWithSelector(MockMulticall(multicall).functionThatRevertsWithString.selector, "First call failed");
        calls[1] = abi.encodeWithSelector(MockMulticall(multicall).functionThatReturnsTuple.selector, 1, 2);
    ...
    function test_multicall_secondRevert() public {
        ...
        calls[1] = abi.encodeWithSelector(MockMulticall(multicall).functionThatRevertsWithString.selector, "Second call failed");
    ...
    function test_multicall_bubbleRevert_string() public {
        bytes[] memory calls = new bytes[](1);
        calls[0] = abi.encodeWithSelector(MockMulticall(multicall).functionThatRevertsWithString.selector, "errorString");
